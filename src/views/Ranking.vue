    ...
    if (Array.isArray(jugador.partidas)) {
      jugador.partidas.forEach(partida => {
        partidasIndividuales.push({
          ...
          puntos: Number(partida.puntos || jugador.puntos || 0),
          tiempoTotal: Number(partida.tiempoTotal || jugador.tiempoTotal || 0),

          // ✅ AQUÍ YA NO FORZAMOS 'medio'
          nivel: (partida.nivel ?? jugador.nivel ?? '').toString().trim(),
      ...
      partidasIndividuales.push({
        ...
        tiempoTotal: Number(jugador.tiempoTotal || 0),

        // ✅ AQUÍ TAMPOCO
        nivel: (jugador.nivel ?? '').toString().trim(),
...
const rankingFiltrado = computed(() => {
  if (filtroNivel.value === 'todos') return ranking.value

  return ranking.value.filter(partida =>
    normalizarNivel(partida.nivel) === normalizarNivel(filtroNivel.value)
  )
})
...
const estadisticasNiveles = computed(() => {
  ...
  ranking.value.forEach(partida => {
    const nivel = normalizarNivel(partida.nivel || 'medio')
